class Cobweb::CrawlJob
  ...
  def self.perform(content_request)
    ...
    redis = Cobweb::NamespacedRedis.new(Redis.new(content_request[:redis_options]), "cobweb-#{content_request[:crawl_id]}")
    ...
    unless redis.sismember "crawled", content_request[:url]
      ...
      if crawl_counter <= content_request[:crawl_limit].to_i
        content = Cobweb::Spider.new(content_request).get(content_request[:url])
